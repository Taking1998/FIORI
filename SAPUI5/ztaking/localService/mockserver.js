sap.ui.define(
  ["sap/ui/model/json/JSONModel", "sap/ui/thirdparty/sinon", "sap/base/Log"],
  function (JSONModel, sinon, Log) {
    ...
    var oSandbox = sinon.sandbox.create(),
      ...
      sNamespace = "ztaking",
      sLogComponent = "ztaking.mockserver", // Component for writing logs into the console
    ...
    function readData() {
      var oMetadataPromise = new Promise(function (fnResolve, fnReject) {
        var sResourcePath = sap.ui.require.toUrl(
          sNamespace + "/localService/metadata.xml"
        );
      ...
      var oMockDataPromise = new Promise(function (fnResolve, fnReject) {
        var sResourcePath = sap.ui.require.toUrl(
          sNamespace + "/localService/mockdata/people.json"
        );
    ...
    function handleGetUserRequests(oXhr, bCount) {
      ...
      if (sKey) {
        ...
        aResult.forEach(function (oUser) {
          ...
          oResponse.value.push(
            getUserObject(iUserIndex, aSelect, aExpand, aSubSelects)
          );
        });
    ...
    function handlePatchUserRequests(oXhr) {
      ...
      if (
        oChanges.hasOwnProperty("UserName") &&
      ...
      switch (oXhr.method) {
        case "GET":
          if (/\$metadata/.test(oXhr.url)) {
            ...
            aResponse = handleGetUserRequests(
              oXhr,
              /\$count=true/.test(oXhr.url)
            );
          }
          break;
        case "PATCH":
